...
New-Item -ItemType Directory -Force `
  $outputPath, `
  (Join-Path $outputPath "python"), `
  (Join-Path $outputPath "R3\ckpt"), `
  (Join-Path $outputPath "backend"), `
  (Join-Path $outputPath "prerequisites") | Out-Null
...
Copy-Item $vcRedist (Join-Path $outputPath "prerequisites\vc_redist.x64.exe")
...
$manifest | Add-Member -NotePropertyName bundled_prerequisites -NotePropertyValue @(
  @{
...
$manifest | Add-Member -NotePropertyName external_prerequisites -NotePropertyValue @(
  @{
    name = "NVIDIA display driver"
    reason = "Required by CUDA. It must match the installed NVIDIA GPU and be installed as a Windows system driver."
    minimum_windows_driver = $manifest.minimum_nvidia_driver_windows
